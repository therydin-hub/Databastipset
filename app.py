...
APP_VERSION = "v12.0cr – Cloud-safe budget 2500"
...
# =============================================================================
# v12.0cq – Slutmotor: budget 2500 + profil först
# =============================================================================
# Slutmotorn använder V47-appregeln:
# 1) bygg B52200_TRUE och B52200_BS29_TIGHT med profilfilter först
# 2) tillåt högst några strukturfilter som stöd
# 3) blockera paket över 2 500 rader som slutmotor.
#
# Själva V47-maskineriet ligger i en sidomodul, tipset_final_motor_v47_budget2500_profile_first.py, så appfilen
...
def _v46_engine_module_path():
    try:
        ...
        base = Path.cwd()
    return base / 'tipset_final_motor_v47_budget2500_profile_first.py'
...
def _load_v46_engine_module_cached(path_str):
    path = Path(path_str)
    if not path.exists():
        raise FileNotFoundError(f"Hittar inte V47-motorn: {path.name}. Lägg tipset_final_motor_v47_budget2500_profile_first.py i samma mapp som appen.")
    spec = importlib.util.spec_from_file_location('tipset_final_motor_v47_budget2500_profile_first', str(path))
    if spec is None or spec.loader is None:
        raise RuntimeError('Kunde inte ladda V47-motorn som Python-modul.')
    mod = importlib.util.module_from_spec(spec)
    sys.modules['tipset_final_motor_v47_budget2500_profile_first'] = mod
...
def _v46_final_args(top_n, pay_min, pay_max, filter_hist_target_pct=95):
    """Standardargument från V47: hård budget och profilfilter först."""
    return argparse.Namespace(
        ...
        # V47: profilfilter måste få chans att pressa till <=2500. Strukturfiltret
        # är begränsat inne i motorfilen och får inte starta paketet.
        candidate_min_hit=23, min_candidate_val_pct=50.0, min_structure_val_pct=70.0,
        min_gap_score=0.35, variants_per_key=12, max_candidates=420,
        fast_no_supermakro=True, min_hit=22, min_package_val_pct=54.0,
        min_structure_package_val_pct=76.0, min_unique_rows=6,
        beam_width=125, archive_width=1700, structure_seed_count=1,
        hit_power=2.05, validation_power=0.60, reduction_power=5.10,
...
def _v46_pkg_diag_row(label, pkg=None, error=''):
    ...
    rows = int(pkg.get('frame_after', 0) or 0)
    budget_ok = str(pkg.get('v31_budget_ok', '')).upper() == 'JA' or rows <= 2500
    return {
        'Paket': label,
        'Status': 'OK' if budget_ok else 'ÖVER_BUDGET',
        ...
        'Orsak': 'OK' if budget_ok else 'Blockerad: över 2 500 rader',
    }


def _v46_micro_tight_close_select(base_pkg, tight_pkg):
    """V47-val: välj bara paket under 2 500 rader. Ingen bred fallback godkänns."""
    max_rows = 2500
    candidates = []
    for label, pkg in [('B52200_TRUE', base_pkg), ('B52200_BS29_TIGHT', tight_pkg)]:
        if not isinstance(pkg, dict):
            continue
        rows = int(pkg.get('frame_after', 10**9) or 10**9)
        budget_ok = str(pkg.get('v31_budget_ok', '')).upper() == 'JA' or rows <= max_rows
        if rows <= max_rows and budget_ok:
            candidates.append((label, pkg))

    def key(item):
        label, pkg = item
        rows = int(pkg.get('frame_after', 10**9) or 10**9)
        h = int(pkg.get('hist_hit', 0) or 0)
        v = int(pkg.get('val_hit', 0) or 0)
        profile = int(pkg.get('profile_filters', pkg.get('value_filters', 0)) or 0)
        struct = int(pkg.get('structure_filters', 0) or 0)
        # Budget först, därefter träff/validering. Vi vill inte ha 30/30 på 4 800.
        return (h, v, profile, -struct, -abs(rows - 2200), -rows)

    if candidates:
        selected_label, selected_pkg = max(candidates, key=key)
        return selected_pkg, {
            'selected_source': selected_label,
            'rescue_active': selected_label != 'B52200_TRUE',
            'eligible': True,
            'reason': f'V47 budgetval: valt {selected_label} med {int(selected_pkg.get("frame_after", 0))} rader <= {max_rows}. Strukturfilter={int(selected_pkg.get("structure_filters",0) or 0)}, profilfilter={int(selected_pkg.get("profile_filters",0) or 0)}.',
        }

    # Ingen kandidat får döljas som OK om den är över budget.
    diag = []
    for label, pkg in [('B52200_TRUE', base_pkg), ('B52200_BS29_TIGHT', tight_pkg)]:
        if isinstance(pkg, dict):
            diag.append(f'{label}: {int(pkg.get("frame_after", 0) or 0)} rader, status={pkg.get("v31_budget_status", "")}, träff={pkg.get("hist_hit",0)}/{pkg.get("hist_total",0)}, struktur={pkg.get("structure_filters",0)}, profil={pkg.get("profile_filters",0)}')
        else:
            diag.append(f'{label}: saknas')
    return None, {
        'selected_source': 'INGEN_UNDER_2500',
        'rescue_active': False,
        'eligible': False,
        'reason': 'Inget godkänt V47-paket under 2 500 rader hittades. ' + ' | '.join(diag),
    }

def _normalize_v46_package_for_app(pkg, source_label, decision_meta):
    """Gör V47-paketet kompatibelt med appens paketlista/appliceringslogik."""
    if not isinstance(pkg, dict):
        return pkg
    out = dict(pkg)
    out.setdefault('groups', [])
    out['package_type'] = 'Slutmotor V47 – budget 2500, profil först'
    out['variant'] = 'B52200_BUDGET2500_PROFILE_FIRST'
    out['variant_label'] = 'B52200 budget 2500 + profil först'
    out['target_label'] = f"V47 slutmotor {int(out.get('hist_hit', 0))}/{int(out.get('hist_total', 0))}"
    ...
    meta.update({
        'engine': 'v47_budget2500_profile_first_app',
...
def _run_v46_final_motor_current(sim_df, specs, frame_rows, frame, filter_vec, antal_matcher, global_db=None, top_n=30, pay_min=100000, pay_max=2500000, filter_hist_target_pct=95):
    """Bygger aktuell kupongs slutmotorpaket med V47-maskineriet."""
    if int(antal_matcher) != 13:
        raise ValueError('V47-slutmotorn är endast låst/testad för 13 matcher.')
    ...
    base_args = mod._v15_variant_args(args, 'B52200_TRUE')
    global_settings = {
    ...
    if not candidates:
        raise RuntimeError('V47 kunde inte bygga några giltiga kandidater för aktuell kupong.')
    ...
    if selected is None:
        raise RuntimeError(decision.get('reason', 'V47 kunde inte välja paket.'))
    ...
    meta = {
        'package_engine': 'v47_budget2500_profile_first_app',
...
if st.session_state.get('v12_analysis_ready') and st.session_state.get('v12_frame_saved'):
    ...
    with st.expander("🧠 Rekommenderade filterpaket", expanded=False):
        ...
        st.markdown("**🎯 Slutmotor V47 – budget 2500, profil först**")
        st.caption("V47 testar samma B52200-spår men med hård budget: paket över cirka 2 500 rader blockeras, profilfilter körs före struktur och struktur får bara vara stöd. Detta använder inte facit.")
        v46_warns = []
        if int(antal_matcher) != 13:
        ...
        run_v46_final = st.button("🎯 Beräkna slutmotor V47 budget 2500", use_container_width=True, key="v12_run_v46_final_motor")
        if run_v46_final:
            for _k in ['v12_recommended_packages', 'v12_recommended_candidate_audit', 'v12_recommended_meta', 'v12_applied_package_meta', 'v12_applied_package_snapshot']:
                st.session_state.pop(_k, None)
            try:
                with st.spinner("Kör V47-slutmotorn: B52200_TRUE + tight micro-rescue..."):
                ...
                st.error(f"V47-slutmotorn kunde inte köras: {e}")
