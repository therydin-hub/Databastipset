# -*- coding: utf-8 -*-
"""Svenska Spels externa API för Databastipset.

Databastipsets historik bygger på startoddset omräknat till 1-X-2-värden.
Prioritet per match:

1. ``randomResultProbability`` – Svenska Spels färdiga startoddsvärde.
2. Egen marginalrensad omräkning av ``startOdds``.
3. ``favouriteOdds`` eller egen omräkning av aktuella ``odds`` används endast
   när användaren uttryckligen har godkänt fallback för matcher utan startodds.

``distribution`` är Svenska folkets streck och används bara för visning.
"""
...
DATABASE_FILE_MAP: dict[str, str] = {
    "Stryktips": "Stryktips _Med_Rank.csv",
    "Europatips": "Europatips _Med_Rank.csv",
    "Topptips ST": "ToppTips_St_Med_Rank.csv",
    "Topptips EU": "ToppTips_Eu _Med_Rank.csv",
    "Topptips Övrigt": "ToppTips_Övriga Med_Rank.csv",
    "Powerplay": "PowerPlay_Med_Rank.csv",
}
...
class MatchOddsRow:
    event_number: int
    description: str
    distribution: Optional[tuple[int, int, int]]
    ...
    *,
    ...
    if not key:
        raise SvenskaSpelAccessKeyError(
            "Ingen API-nyckel angavs. Lägg in SVENSKA_SPEL_ACCESSKEY i "
            "Streamlit Secrets eller skriv nyckeln i lösenordsfältet."
        )
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "Databastipset-SvenskaSpel/1.0",
        },
        method="GET",
    )
    ...
    try:
        ...
        raise SvenskaSpelResponseError("API:t svarade inte med giltig JSON.") from exc
...
def parse_odds_triplet(source: Any) -> Optional[tuple[float, float, float]]:
    values = _get_triplet(source, aliases=SIGN_ALIASES)
    if values is None or any(value <= 1.0 for value in values):
        return None
    return values
...
def parse_probability_triplet(source: Any) -> Optional[tuple[int, int, int]]:
    values = _get_triplet(source, aliases=SIGN_ALIASES)
    if values is None or any(value < 0 or value > 100 for value in values):
        return None
    try:
...
def odds_to_values(odds: Sequence[float]) -> tuple[int, int, int]:
    if len(odds) != 3:
...
def select_draw(
    ...
    *,
    ...
    """Väljer den mest sannolika aktuella kupongen om svaret har flera draws."""
    ...
    def score(draw: Mapping[str, Any]) -> tuple[int, int, float, float]:
        ...
        draw_number = _parse_decimal(
            draw.get("drawNumber") or draw.get("number") or draw.get("drawNo")
        )
...
def extract_match_rows(draw: Mapping[str, Any]) -> list[MatchOddsRow]:
    ...
    for fallback_number, raw_event in enumerate(events, start=1):
        ...
        distribution = parse_probability_triplet(raw_event.get("distribution"))
        ...
        event_status = str(
            raw_event.get("sportEventStatus") or raw_event.get("status") or ""
        ).strip()
...
def build_coupon_vector(
    ...
    *,
    ...
    """Bygger vektorn 1-X-2 per match och redovisar källa samt olösta matcher."""
    vector: list[int] = []
    sources: list[str] = []
    unresolved: list[int] = []
