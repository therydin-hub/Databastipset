...
# =============================================================================
# V47 APP OVERRIDE – STRICT BUDGET 2500 + PROFILE FIRST
# =============================================================================
# Denna override används av Streamlit-appen. Målet är att inte acceptera breda
# 30/30-fallbackpaket på 4–5k rader när användaren uttryckligen vill ha cirka
# <= 2500 paketrader. Struktur används bara som efterstöd, inte som huvudmotor.

V47_STRICT_BUDGET_MAX_ROWS = 2500
V47_STRICT_BUDGET_MIN_ROWS = 1700
V47_MAX_STRUCTURE_FILTERS = 3
V47_MIN_PROFILE_FILTERS = 1

try:
    _ORIG_V46_BUILD_CLUSTER_PACKAGE = _build_cluster_payout_package_v13_from_candidates
except Exception:
    _ORIG_V46_BUILD_CLUSTER_PACKAGE = None

try:
    _ORIG_V46_V31_SELECT_STATE = _v31_select_state
except Exception:
    _ORIG_V46_V31_SELECT_STATE = None


def _v47_is_app_strict_variant(variant_id: str) -> bool:
    return str(variant_id or '').upper() in {'B52200_TRUE', 'B52200_BS29_TIGHT'}


def _v47_budget_bounds(args, variant_id: str):
    cfg = _v31_cfg_for_variant(variant_id) if '_v31_cfg_for_variant' in globals() else {}
    # Appens hårda tak: aldrig högre än 2500 i slutmotorn.
    hi = min(int(cfg.get('max_rows', V47_STRICT_BUDGET_MAX_ROWS) or V47_STRICT_BUDGET_MAX_ROWS), V47_STRICT_BUDGET_MAX_ROWS)
    lo = int(cfg.get('min_rows', V47_STRICT_BUDGET_MIN_ROWS) or V47_STRICT_BUDGET_MIN_ROWS)
    lo = min(lo, hi)
    target = int(cfg.get('target_rows', (lo + hi) // 2) or ((lo + hi) // 2))
    target = max(lo, min(target, hi))
    return lo, target, hi


def _v47_structure_limited_expand(start_states, structure_cands, *, hist_floor, val_floor_hit, sign_bits, min_unique_rows, payout_values, args, variant_id, max_structure_filters=V47_MAX_STRUCTURE_FILTERS):
    """Liten struktur-expansion efter profilpaket. Max 3 strukturfilter."""
    if not structure_cands or int(max_structure_filters) <= 0:
        return []
    states = list(start_states)
    archive = []
    max_depth = int(max_structure_filters)
    for depth in range(max_depth):
        expanded = []
        for st in states:
            if int(st.structure_filters) >= int(max_structure_filters):
                continue
            for cand in structure_cands:
                ns2 = _v13_apply_sequence(
                    st, [cand], phase='structure', hist_floor=hist_floor,
                    val_floor_hit=val_floor_hit, sign_bits=sign_bits,
                    min_unique_rows=min_unique_rows,
                )
                if ns2 is None:
                    continue
                if int(ns2.structure_filters) > int(max_structure_filters):
                    continue
                expanded.append(ns2)
        if not expanded:
            break
        states = _v13_dedupe(expanded, payout_values, args, min(int(args.beam_width), 90), variant_id)
        archive = _v13_dedupe(archive + states, payout_values, args, min(int(args.archive_width), 600), variant_id)
    return archive


def _v47_state_budget_key(st, hist_payout, args, variant_id: str):
    lo, target, hi = _v47_budget_bounds(args, variant_id)
    rows = int(st.frame_count)
    m = _v13_state_metrics(st, hist_payout, args, variant_id)
    in_band = 1 if lo <= rows <= hi else 0
    under_hi = 1 if rows <= hi else 0
    profile_filters = int(getattr(st, 'profile_filters', 0))
    structure_filters = int(getattr(st, 'structure_filters', 0))
    # Inom budget först. Därefter träff/validering. Struktur får bara vara stöd.
    return (
        under_hi,
        in_band,
        int(st.hist_hit),
...
def _v47_select_strict_budget_state(valid_states, hist_payout, args, variant_id: str):
    lo, target, hi = _v47_budget_bounds(args, variant_id)
    states = [s for s in list(valid_states) if int(s.profile_filters) >= V47_MIN_PROFILE_FILTERS and int(s.structure_filters) <= V47_MAX_STRUCTURE_FILTERS]
    if not states:
        states = [s for s in list(valid_states) if int(s.structure_filters) <= V47_MAX_STRUCTURE_FILTERS]
    if not states:
        states = list(valid_states)

    in_band = [s for s in states if lo <= int(s.frame_count) <= hi]
    under_hi = [s for s in states if int(s.frame_count) <= hi]
    over_hi = [s for s in states if int(s.frame_count) > hi]
    pool = in_band or under_hi or over_hi or states
    best = max(pool, key=lambda s: _v47_state_budget_key(s, hist_payout, args, variant_id))

    if in_band:
        status, ok = 'V47_INOM_BUDGET_PROFIL_FÖRST', 'JA'
    elif under_hi:
        status, ok = 'V47_UNDER_MAX_PROFIL_FÖRST', 'JA'
    else:
        status, ok = 'V47_ÖVER_BUDGET_EJ_GODKÄND', 'NEJ'
    return best, {
        'status': status,
        'ok': ok,
        'target': int(target),
        'min': int(lo),
        'max': int(hi),
        'delta': int(int(best.frame_count) - int(target)),
        'pool_in_band': int(len(in_band)),
        'pool_under_max': int(len(under_hi)),
        'pool_over_max': int(len(over_hi)),
        'strict_budget': True,
        'max_structure_filters': int(V47_MAX_STRUCTURE_FILTERS),
        'profile_first': True,
    }


def _build_cluster_payout_package_v13_from_candidates(ns: dict, candidates: List[dict], htot: int, vtot: int, ftot: int, hist_payout: np.ndarray, frame_rows: list, frame: list, antal_matcher: int, args, variant_id: str):  # noqa: F811
    """V47 appoverride för B52200-varianter. Övriga varianter går till original."""
    if not _v47_is_app_strict_variant(variant_id):
        if _ORIG_V46_BUILD_CLUSTER_PACKAGE is None:
            return None, {'error': 'Originalbyggare saknas.'}
        return _ORIG_V46_BUILD_CLUSTER_PACKAGE(ns, candidates, htot, vtot, ftot, hist_payout, frame_rows, frame, antal_matcher, args, variant_id)
    ...
    val_floor_profile = int(math.ceil(vtot * float(args.min_package_val_pct) / 100.0)) if vtot else 0
    val_floor_structure = int(math.ceil(vtot * float(getattr(args, 'min_structure_package_val_pct', args.min_package_val_pct)) / 100.0)) if vtot else 0

    structure_cands = [c for c in candidates if _is_structure_candidate(c) and int(c.get('hist_hit', 0)) == htot]
    profile_cands = [c for c in candidates if _is_profile_candidate(c) and int(c.get('hist_hit', 0)) >= int(args.min_hit)]
    if not profile_cands:
        return None, {'error': f'V47 hittade inga profilkandidater för {variant_id}.', 'candidates': len(candidates)}

    # 1) Profil först – ingen struktur får starta paketet.
    print(f'    V47 {variant_id}: profil först · profilkandidater={len(profile_cands)} · strukturkandidater={len(structure_cands)}', flush=True)
    profile_states = _v13_beam_search(
        [initial], profile_cands,
        phase='profile', hist_floor=int(args.min_hit),
        val_floor_hit=val_floor_profile,
        sign_bits=sign_bits, min_unique_rows=int(args.min_unique_rows),
        payout_values=hist_payout, args=args, variant_id=variant_id,
    ...
    # 2) Struktur får bara hjälpa på slutet, max 3 filter.
    profile_seed_pool = [s for s in profile_states if s.chosen and int(s.profile_filters) >= V47_MIN_PROFILE_FILTERS]
    profile_seed_pool = _v13_dedupe(profile_seed_pool, hist_payout, args, min(int(args.beam_width), 100), variant_id)
    structure_after_profile = _v47_structure_limited_expand(
        profile_seed_pool[:min(40, len(profile_seed_pool))], structure_cands,
        hist_floor=int(args.min_hit),
        val_floor_hit=max(val_floor_profile, min(val_floor_structure, vtot)) if vtot else 0,
        sign_bits=sign_bits, min_unique_rows=int(args.min_unique_rows),
        payout_values=hist_payout, args=args, variant_id=variant_id,
        max_structure_filters=V47_MAX_STRUCTURE_FILTERS,
    )

    finals = []
    finals.extend(profile_states)
    finals.extend(structure_after_profile)
    valid = [
        st for st in finals
        if st.chosen
        and int(st.hist_hit) >= int(args.min_hit)
        and (vtot == 0 or int(st.val_hit) >= int(val_floor_profile))
        and int(st.profile_filters) >= V47_MIN_PROFILE_FILTERS
        and int(st.structure_filters) <= V47_MAX_STRUCTURE_FILTERS
    ]
    if not valid:
        return None, {'error': f'Ingen V47-state klarade profil/budgetkraven för variant {variant_id}.', 'candidates': len(candidates), 'profile_candidates': len(profile_cands)}

    valid = _v13_dedupe(valid, hist_payout, args, int(args.archive_width), variant_id)
    best, budget_meta = _v47_select_strict_budget_state(valid, hist_payout, args, variant_id)
    best = _v9_redundancy_cleanup(best, hist_floor=int(args.min_hit), val_floor_hit=val_floor_profile, sign_bits=sign_bits)
    # Cleanup kan ta bort filter och ändra budgetutfall; räkna om.
    lo, target, hi = _v47_budget_bounds(args, variant_id)
    budget_meta['delta'] = int(int(best.frame_count) - int(target))
    budget_meta['ok'] = 'JA' if int(best.frame_count) <= int(hi) else 'NEJ'
    if budget_meta['ok'] != 'JA':
        budget_meta['status'] = 'V47_ÖVER_BUDGET_EJ_GODKÄND'
    elif int(best.frame_count) < int(lo):
        budget_meta['status'] = 'V47_UNDER_MIN_MEN_UNDER_MAX'
    ...
    package = {
        ...
        'variant_label': V13_VARIANT_DEFS.get(str(variant_id), {}).get('label', str(variant_id)),
        'target': int(best.hist_hit),
        'target_label': f'V47 {variant_id} {best.hist_hit}/{htot}',
        'hist_hit': int(best.hist_hit), 'hist_total': int(htot),
        'val_hit': int(best.val_hit), 'val_total': int(vtot),
        'frame_start': int(ftot), 'frame_after': int(best.frame_count),
        'reduction_pct': float(metrics['reduction_pct']),
        ...
        'num_filters': len(best.chosen),
        'filters': list(best.chosen), 'steps': list(best.steps),
        'package_type': f'V47 {variant_id} – budget 2500, profil först',
        'structure_filters': best.structure_filters,
        'profile_filters': best.profile_filters,
        'fat_filters': best.fat_filters,
        'edge_filters': best.edge_filters,
        'v31_budget_target': int(target),
        'v31_budget_min': int(lo),
        'v31_budget_max': int(hi),
        'v31_budget_status': str(budget_meta.get('status','')),
        'v31_budget_ok': str(budget_meta.get('ok','')),
        'v31_budget_delta': int(budget_meta.get('delta', 0) or 0),
        'v31_pool_in_band': int(budget_meta.get('pool_in_band', 0) or 0),
        'v31_pool_under_max': int(budget_meta.get('pool_under_max', 0) or 0),
        'meta': {
            'engine': 'v47_budget2500_profile_first',
            'variant': str(variant_id),
            'variant_label': V13_VARIANT_DEFS.get(str(variant_id), {}).get('label', str(variant_id)),
            'min_hit': int(args.min_hit),
            'min_package_val_pct': float(args.min_package_val_pct),
            ...
            'archive_width': int(args.archive_width),
            'v31_budget': budget_meta,
            'strict_max_rows': int(V47_STRICT_BUDGET_MAX_ROWS),
            'max_structure_filters': int(V47_MAX_STRUCTURE_FILTERS),
            'profile_first': True,
        },
    }
